...
TEST(DKVRCLI, CLI)
{
    dkvr::DKVRCLI dkvr(true);

    dkvr.Run();
    ASSERT_EQ(dkvr.is_running(), true);

    while (dkvr.is_running())
    {
        std::string str;
...
//std::atomic_bool shown[3] = { true, true, true };
...
//	dkvrCreateInstance(&handle, init_result, sizeof init_result);
//	ASSERT_NE(handle, nullptr);
//
//	// run instance and assert
//	DKVRAddress addr{ 0 };
//	dkvrRunHost(handle, addr);
...
//			int raw = 0;
//			int nominal = 0;
//			dkvrTrackerGetBehaviorLed(handle, i, &led);
//			dkvrTrackerGetBehaviorActive(handle, i, &active);
//			dkvrTrackerGetBehaviorRaw(handle, i, &raw);
//			dkvrTrackerGetBehaviorNominal(handle, i, &nominal);
//			int behavior = (nominal << 3) | (raw << 2) | (active < 1) | led;
//
//			unsigned char* ptr = reinterpret_cast<unsigned char*>(&address);
//			std::cout << i << '\t' << (int)ptr[0] << "." << (int)ptr[1] << "." << (int)ptr[2] << "." << (int)ptr[3] << '\t' << con_str << '\t'
//				<< "[" << (led ? "led " : "") << (active ? "active " : "") << (raw ? "raw " : "") << (nominal ? "nominal" : "") << "]"
//				<< std::endl;
//		}
//	}
...
//
//		int led		= behavior & 0b0001;
//		int active	= behavior & 0b0010;
//		int raw		= behavior & 0b0100;
//		int nominal = behavior & 0b1000;
//		dkvrTrackerSetBehaviorLed(handle, target, led);
//		dkvrTrackerSetBehaviorActive(handle, target, active);
//		dkvrTrackerSetBehaviorRaw(handle, target, raw);
//		dkvrTrackerSetBehaviorNominal(handle, target, nominal);
...
//			{
//				dkvrCalibratorGetSampleTypeString(handle, buffer, sizeof(buffer));
...
//		DKVRQuaternion quat{};
//		DKVRVector3 linear_accel{}, mag_dist{};
//
//		dkvrTrackerGetRawGyro(handle, target, &gyro);
//		dkvrTrackerGetRawAccel(handle, target, &accel);
//		dkvrTrackerGetRawMag(handle, target, &mag);
//		dkvrTrackerGetOrientation(handle, target, &quat);
//		dkvrTrackerGetLinearAcceleration(handle, target, &linear_accel);
//		dkvrTrackerGetMagneticDisturbance(handle, target, &mag_dist);
//
//		if (imu_ypr)
//		{
...
//			if (shown[1])
//				std::cout << std::setprecision(3) << std::fixed << "LA  : (" << linear_accel.x << ", " << linear_accel.y << ", " << linear_accel.z << ")" << std::endl;
//			if (shown[2])
//				std::cout << std::setprecision(3) << std::fixed << "MD  : (" << mag_dist.x << ", " << mag_dist.y << ", " << mag_dist.z << ")" << std::endl;
//
